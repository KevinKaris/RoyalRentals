...
if(isset($_POST['f_name']) && isset($_POST['l_name']) && isset($_POST['username']) && isset($_POST['phone']) && isset($_POST['email']) && isset($_POST['password']) && !isset($_POST['image'])){
    ...
    $sql = "UPDATE users SET f_name = ?, l_name = ?, username = ?, phone = ?, email = ?, password = ? WHERE id = ? AND user_group = ?";

    $statement = $con -> prepare($sql);
    $data = [$f_name, $l_name, $username, $phone, $email, $password, $user_id, '1'];
    ...
    $sql = "UPDATE users SET f_name = ?, l_name = ?, username = ?, phone = ?, email = ? WHERE id = ? AND user_group = ?";

    $statement = $con -> prepare($sql);
    $data = [$f_name, $l_name, $username, $phone, $email, $user_id, '1'];
    $st = $statement -> execute($data);
    ...
    // $SELECT = "SELECT photo FROM users WHERE id = ? AND user_group = ?";
    // $statement2 = $con -> prepare($SELECT);
    // $statement2 -> execute([$user_id, '1']);
    ...
    // $sql = "UPDATE users SET photo = ? WHERE id = ? AND user_group = ?";
    // $statement = $con -> prepare($sql);
    // $st = $statement -> execute([$image_name, $user_id, '1']);
